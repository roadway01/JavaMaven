...
public class TThreadedSelectorClientDemo {
	...
	public static void main(String args[]) throws InterruptedException{
		TThreadedSelectorClientDemo clientDemo = new TThreadedSelectorClientDemo();
		CountDownLatch latch = new CountDownLatch(3);
		long start = System.currentTimeMillis();
		for(int i = 0; i < 3; i++){
			int threadId = i;
			new Thread(() -> {
				clientDemo.startClient("threadedSelector" + threadId);
				latch.countDown();
			}).start();
		}
		latch.await();
		long end = System.currentTimeMillis();
		System.out.println("execute complete:" + (end - start));
	}
}
